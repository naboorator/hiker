...
export function registerFriendDeleteRoutes(router) {
    router.delete('/friends/requests/:requestId', async (request, response) => {
        ...
        await database.update((data) => {
            const index = data.friendConnections.findIndex((connection) => connection.id === request.params['requestId'] &&
    ...
    router.delete('/friends/:friendId', async (request, response) => {
        const userId = authenticatedUserId(response);
        await database.update((data) => {
            const index = data.friendConnections.findIndex((connection) => connection.userIds.includes(userId) &&
                connection.status !== 'pending' &&
