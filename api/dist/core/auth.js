...
export async function requireAuthentication(request, response, next) {
    ...
    try {
        ...
        const [user] = await database.query("SELECT role, status, email_confirmed AS emailConfirmed FROM users WHERE id = ?", [payload.sub]);
        if (!user)
        ...
        if (!user.emailConfirmed)
            return next(new HttpError(403, "Email address has not been confirmed", "EMAIL_NOT_CONFIRMED"));
