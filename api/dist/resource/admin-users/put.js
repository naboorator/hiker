...
export function registerAdminUserPutRoutes(router) {
    ...
    router.put("/admin/users/:id", async (request, response) => {
        ...
        try {
            const user = await withTransaction(async (connection) => {
                const [current] = await connection.query(`SELECT name, email_confirmed AS emailConfirmed, CONCAT(
             DATE_FORMAT(created_at, '%Y-%m-%dT%H:%i:%s.'),
             LEFT(DATE_FORMAT(created_at, '%f'), 3), 'Z'
           ) AS createdAt, status
             FROM users WHERE id = ? AND status <> 'deleted' FOR UPDATE`, [request.params["id"]]);
                if (!current)
                    throw new HttpError(404, "User not found");
                await connection.query(`UPDATE users
              SET name = ?, email = ?, role = ?
            WHERE id = ?`, [input.name, input.email, input.role, request.params["id"]]);
                if (current.name.toLocaleLowerCase() !== input.name.toLocaleLowerCase()) {
                    await connection.query("UPDATE settings SET owner_name = ? WHERE user_id = ?", [input.name, request.params["id"]]);
                    await connection.query(`UPDATE activity_people ap
               JOIN activities a ON a.id = ap.activity_id
                SET ap.person_name = ?
              WHERE a.user_id = ? AND LOWER(ap.person_name) = LOWER(?)`, [input.name, request.params["id"], current.name]);
                }
                return {
...
async function updateStatus(userId, status, response) {
    requireAdministrator(response);
    if (userId === authenticatedUserId(response))
        throw new HttpError(400, "You cannot block or unblock your own account");
    const result = await withTransaction(async (connection) => {
        const update = await connection.query("UPDATE users SET status = ? WHERE id = ? AND status <> 'deleted'", [status, userId]);
        if (!update.affectedRows)
            throw new HttpError(404, "User not found");
        const [user] = await connection.query(`SELECT id, name, email, role, status,
