...
export function registerAdminUserDeleteRoutes(router: Router): void {
  router.delete("/admin/users/:id", async (request, response) => {
    ...
    await withTransaction(async (connection) => {
      ...
      await connection.query(
        "DELETE FROM friend_connections WHERE user_id_1 = ? OR user_id_2 = ?",
        [userId, userId],
      );
      await connection.query(
        "DELETE FROM activity_reactions WHERE user_id = ?",
        [userId],
      );
      await connection.query(
        `UPDATE users
