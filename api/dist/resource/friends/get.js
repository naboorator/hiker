...
export function registerFriendGetRoutes(router) {
    router.get('/friends/comparison', async (request, response) => {
        const userId = authenticatedUserId(response);
        const currentMonth = new Date().toISOString().slice(0, 7);
        const month = typeof request.query['month'] === 'string' && /^\d{4}-(0[1-9]|1[0-2])$/.test(request.query['month'])
            ? request.query['month']
            : currentMonth;
        const friendIds = [
            ...new Set((typeof request.query['friendIds'] === 'string' ? request.query['friendIds'] : '')
                .split(',')
                .map((id) => id.trim())
                .filter(Boolean)),
        ].slice(0, 50);
        ...
        if (friendIds.length) {
            ...
            const [allowed] = await database.query(`SELECT COUNT(*) AS total
           FROM friend_connections
          WHERE status = 'accepted'
            AND ((user_id_1 = ? AND user_id_2 IN (${placeholders}))
              OR (user_id_2 = ? AND user_id_1 IN (${placeholders})))`, [userId, ...friendIds, userId, ...friendIds]);
        ...
        const users = await database.query(`SELECT id, name FROM users WHERE id IN (${placeholders})`, selectedIds);
        const counts = await database.query(`SELECT user_id AS userId, CAST(activity_date AS CHAR) AS date,
              COUNT(*) AS activityCount
         FROM activities
        WHERE user_id IN (${placeholders})
          AND DATE_FORMAT(activity_date, '%Y-%m') = ?
        GROUP BY user_id, activity_date
        ORDER BY activity_date`, [...selectedIds, month]);
        ...
        response.json(selectedIds.map((id) => ({
            userId: id,
    ...
    router.get('/friends/search', async (request, response) => {
        ...
        const users = await database.query(`SELECT u.id, u.name, u.email, f.status AS connectionStatus
         FROM users u
        ...
        LIMIT 20`, [userId, userId, userId, search, search]);
        response.json(users.map((user) => ({ ...user, connectionStatus: user.connectionStatus ?? 'none' })));
    });
    router.get('/friends', async (_request, response) => {
        const userId = authenticatedUserId(response);
        response.json(await database.query(`SELECT u.id, u.name, u.email, u.role
    ...
    router.get('/friends/requests', async (_request, response) => {
        const userId = authenticatedUserId(response);
        const rows = await database.query(`SELECT f.id, f.requester_id AS requesterId, CAST(f.created_at AS CHAR) AS createdAt,
        ...
        ORDER BY f.created_at DESC`, [userId, userId, userId]);
