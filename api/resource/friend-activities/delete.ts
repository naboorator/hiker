...
export function registerFriendActivityDeleteRoutes(router: Router): void {
  router.delete(
    "/friends/activities/:activityId/reactions/:type",
    async (request, response) => {
      const userId = authenticatedUserId(response);
      if (
        request.params["type"] !== "like" &&
      ...
      const result = await database.query(
        ...
        [activityId, userId, request.params["type"]],
      );
      if (!result.affectedRows) throw new HttpError(404, "Reaction not found");
