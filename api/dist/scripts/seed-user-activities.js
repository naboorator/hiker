...
function randomDate() {
    ...
    const date = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate() - randomInt(0, 366)));
    return date.toISOString().slice(0, 10);
}
try {
    const users = await database.query(`SELECT id, name
       FROM users
    ...
    await withTransaction(async (connection) => {
        for (const user of users) {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                ...
                await connection.query(`INSERT INTO activity_people (activity_id, position, person_name)
           VALUES (?, 0, ?)`, [activityId, user.name]);
            }
        }
    });
