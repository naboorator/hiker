...
export function activityInput(value: unknown): ActivityInput {
  ...
  if (minutes < 0 || (metres !== null && metres < 0)) invalid('Values cannot be negative');
  return { activityType: activityType as ActivityType, name, date, minutes, metres, people };
}
...
export function weightInput(value: unknown): WeightInput {
  ...
  if (weightKg < 20 || weightKg > 500) invalid('Weight must be between 20 and 500 kg');
