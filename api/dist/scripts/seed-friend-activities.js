...
try {
    ...
    if (!target)
        throw new Error(`User ${targetEmail} was not found`);
    const friends = await database.query(`SELECT u.id, u.name, u.email
       FROM friend_connections f
       JOIN users u ON u.id = IF(f.user_id_1 = ?, f.user_id_2, f.user_id_1)
      WHERE f.status = 'accepted'
        AND (f.user_id_1 = ? OR f.user_id_2 = ?)
        AND u.status = 'active'
      ORDER BY u.name`, [target.id, target.id, target.id]);
    const results = [];
