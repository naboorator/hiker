...
export function registerAuthPostRoutes(router) {
    router.post("/auth/register", registrationRateLimit, async (request, response) => {
        ...
        const confirmationExpiresAt = new Date(Date.now() + emailConfirmationExpiryHours * 60 * 60_000);
        const created = {
            id: randomUUID(),
            name,
        ...
        try {
            await withTransaction(async (connection) => {
             ...
             VALUES (?, ?, ?, ?, ?, ?, 0, ?)`, [
                ...
                await connection.query(`INSERT INTO email_confirmation_tokens
        ...
        const user = created;
        await deliverTemplatedEmailSafely(emailService, "confirm-registration", language, created.email, `confirm-registration:${confirmationTokenId}`, {
            name: created.name,
    ...
    router.post("/auth/login", async (request, response) => {
        ...
        if (!user.emailConfirmed)
            throw new HttpError(403, "Email address has not been confirmed", "EMAIL_NOT_CONFIRMED");
    ...
    router.post("/auth/confirm-email", async (request, response) => {
        ...
        const confirmed = await withTransaction(async (connection) => {
          ...
          FOR UPDATE`, [hashToken(token)]);
            if (!record)
                return false;
            await connection.query("UPDATE users SET email_confirmed = 1, email_confirmed_at = NOW() WHERE id = ?", [record.userId]);
            await connection.query("UPDATE email_confirmation_tokens SET used_at = NOW() WHERE user_id = ? AND used_at IS NULL", [record.userId]);
            return true;
        });
        if (!confirmed)
    ...
    router.post("/auth/resend-confirmation", confirmationRateLimit, async (request, response) => {
        ...
        const [user] = await database.query(`SELECT id, name, email, language FROM users
          WHERE email = ? AND status = 'active' AND email_confirmed = 0`, [email]);
        if (user) {
            const token = randomBytes(32).toString("base64url");
            const tokenId = randomUUID();
            const expiresAt = new Date(Date.now() + emailConfirmationExpiryHours * 60 * 60_000);
            await withTransaction(async (connection) => {
                await connection.query("UPDATE email_confirmation_tokens SET used_at = NOW() WHERE user_id = ? AND used_at IS NULL", [user.id]);
                await connection.query(`INSERT INTO email_confirmation_tokens
               (id, user_id, token_hash, expires_at, created_at)
             VALUES (?, ?, ?, ?, NOW())`, [
                    tokenId,
            ...
            await deliverTemplatedEmailSafely(emailService, "confirm-registration", language, user.email, `confirm-registration:${tokenId}`, {
                name: user.name,
        ...
        response.status(202).json({
            message: "If this account requires confirmation, a new email has been sent",
        });
    });
