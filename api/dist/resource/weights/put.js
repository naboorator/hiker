...
export function registerWeightPutRoutes(router) {
    router.put('/weights/:id', async (request, response) => {
        ...
        const result = await database.query(`UPDATE weights SET weight_kg = ?, recorded_on = ? WHERE id = ?${administrator ? '' : ' AND user_id = ?'}`, [input.weightKg, input.recordedOn, request.params['id'], ...(administrator ? [] : [userId])]);
