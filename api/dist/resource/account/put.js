...
export function registerAccountPutRoutes(router) {
    router.put("/account/password", async (request, response) => {
        const userId = authenticatedUserId(response);
        const { currentPassword, newPassword, repeatPassword } = passwordInput(request.body);
        if (newPassword !== repeatPassword)
            throw new HttpError(400, "New passwords do not match");
        if (currentPassword === newPassword)
            throw new HttpError(400, "New password must be different from the current password");
        const [user] = await database.query("SELECT password_hash AS passwordHash FROM users WHERE id = ? AND status = 'active'", [userId]);
        if (!user)
