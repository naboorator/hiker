  ...
  router.put("/admin/users/:id", async (request, response) => {
    ...
    try {
      const user = await withTransaction(async (connection) => {
        ...
        >(
          `SELECT name, email_confirmed AS emailConfirmed, CONCAT(
        ...
        await connection.query(
          `UPDATE users
              SET name = ?, email = ?, role = ?
            WHERE id = ?`,
          [input.name, input.email, input.role, request.params["id"]],
        );
        if (
          current.name.toLocaleLowerCase() !== input.name.toLocaleLowerCase()
        ) {
          await connection.query(
            "UPDATE settings SET owner_name = ? WHERE user_id = ?",
            [input.name, request.params["id"]],
          );
          await connection.query(
...
function adminUserInput(value: unknown): {
  name: string;
  email: string;
  role: UserRole;
} {
