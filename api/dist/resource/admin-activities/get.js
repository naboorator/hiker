...
export function registerAdminActivityGetRoutes(router) {
    router.get("/admin/activities", async (request, response) => {
        ...
        const [[activityCount], [dayCount]] = await Promise.all([
            database.query("SELECT COUNT(*) AS total FROM activities"),
            database.query("SELECT COUNT(DISTINCT activity_date) AS total FROM activities"),
        ]);
        ...
        const dates = await database.query(`SELECT CAST(activity_date AS CHAR) AS date
         FROM activities
        GROUP BY activity_date
        ORDER BY activity_date DESC
        LIMIT ? OFFSET ?`, [pageSize, (normalizedPage - 1) * pageSize]);
        let items = [];
        if (dates.length) {
          ...
          ORDER BY a.activity_date DESC, a.created_at DESC, a.id DESC`, dates.map(({ date }) => date));
            const withAuthors = rows.map(({ authorId, authorName, ...activity }) => ({
                ...activity,
                author: { id: authorId, name: authorName },
            }));
            items = await activitiesWithReactionSummaries(await attachPeople(withAuthors));
        }
        const result = {
            items,
            page: normalizedPage,
