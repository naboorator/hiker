...
export function registerWeightGetRoutes(router: Router): void {
  router.get('/weights', async (_request, response) => {
    const userId = authenticatedUserId(response);
    const weights = (await database.read()).weights.filter((weight) => weight.userId === userId).sort(
  ...
  router.get('/weights/:id', async (request, response) => {
    const userId = authenticatedUserId(response);
    const weight = (await database.read()).weights.find(
      ({ id, userId: ownerId }) => id === request.params['id'] && ownerId === userId,
    );
    if (!weight) throw new HttpError(404, 'Weight measurement not found');
