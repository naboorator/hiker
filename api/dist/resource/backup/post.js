...
export function registerBackupPostRoutes(router) {
    router.post("/backup/import", async (request, response) => {
        ...
        const result = await withTransaction(async (connection) => {
            ...
            if (payload.settings) {
                ...
                await connection.query(`UPDATE activity_people p
             JOIN activities a ON a.id = p.activity_id
              SET p.person_name = ?
            WHERE a.user_id = ? AND LOWER(p.person_name) = LOWER(?)`, [settings.ownerName, userId, user.name]);
           ...
           ON DUPLICATE KEY UPDATE app_name = VALUES(app_name), owner_name = VALUES(owner_name)`, [userId, settings.appName, settings.ownerName]);
            ...
            for (const candidate of payload.activities) {
                try {
                    ...
                    const existing = await connection.query(`SELECT id FROM activities
            ...
            for (const candidate of payload.weights) {
                try {
                    ...
                    const existing = await connection.query(`SELECT id FROM weights
              WHERE user_id = ? AND weight_kg = ? AND recorded_on = ?
              LIMIT 1`, [userId, input.weightKg, input.recordedOn]);
                    if (existing.length) {
                        skippedWeights++;
                        continue;
                    }
                    await connection.query(`INSERT INTO weights (id, user_id, weight_kg, recorded_on, created_at)
             VALUES (?, ?, ?, ?, ?)`, [
                        randomUUID(),
                        userId,
                        input.weightKg,
