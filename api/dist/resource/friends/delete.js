...
export function registerFriendDeleteRoutes(router) {
    router.delete('/friends/requests/:requestId', async (request, response) => {
        const userId = authenticatedUserId(response);
        const result = await database.query(`DELETE FROM friend_connections
        WHERE id = ? AND status = 'pending' AND (user_id_1 = ? OR user_id_2 = ?)`, [request.params['requestId'], userId, userId]);
    ...
    router.delete('/friends/:friendId', async (request, response) => {
        ...
        const result = await database.query(`DELETE FROM friend_connections
        WHERE user_id_1 = ? AND user_id_2 = ? AND status = 'accepted'`, [userId1, userId2]);
        if (!result.affectedRows)
